...
argparse "$@" <<EOF || exit 1
parser.add_argument('-t','--tumour-bam', required=True, help='Path to tumour BAM')
...
parser.add_argument('-f', '--somatic-vcf', required=True, help='Path to somatic VCF file')
...
if [ -z ${TUMOUR_BAM} ]; then
  echo "Missing -t|--tumour-bam parameter"
  exit
fi
...
if [ -z ${SOMATIC_VCF} ]; then
  echo "Missing -f|--somatic-vcf parameter"
  exit
fi
...
prepare_function() {
  ...
  if [ ! -f ${TUMOUR_BAM} ]; then
      echo "ERROR: ${TUMOUR_BAM} does not exist"
      exit
  elif [ ! -f ${TUMOUR_BAM}.bai ] && [ ! -f ${TUMOUR_BAM/.bam/.bai} ]; then
    echo "ERROR: ${TUMOUR_BAM} is not indexed"
    exit
  else
  ...
  if [ ! -f ${SOMATIC_VCF} ]; then echo "ERROR: Somatic VCF ${SOMATIC_VCF} does not exist"; exit; fi
  ...
  SAMPLE=$(samtools view -H ${TUMOUR_BAM} | grep '^@RG' | sed "s/.*SM:\([^\t]*\).*/\1/g" | uniq | head -1)
...
remap_tumour_function() {
  REMAP_HLA_T_DIR=${TMPDIR}/tumour
  REMAP_HLA_T_TMP_DIR=${REMAP_HLA_T_DIR}/tmp
  REMAP_HLA_T_OUT=${REMAP_HLA_T_DIR}/${SAMPLE}.tumour.hla.sorted.bam
  REMAP_HLA_T_DONE=${REMAP_HLA_T_OUT}.done
  REMAP_HLA_T_JOB="REMAP_HLA_T_${RAND}"
  REMAP_HLA_T_FLAG=true
  if [ -e ${REMAP_HLA_T_DONE} ]; then
		echo "REMAP_HLA_T output exists at ${REMAP_HLA_T_OUT}"
		REMAP_HLA_T_FLAG=false
	else
		echo "Creating REMAP_HLA_T output directory in ${REMAP_HLA_T_DIR}"
    mkdir -p ${REMAP_HLA_T_DIR} ${REMAP_HLA_T_TMP_DIR}
    REMAP_HLA_T_CMD="samtools view -huL ${HLA_ALTS} ${TUMOUR_BAM} | \
samtools collate -Oun128 -@ 2 - \
${REMAP_HLA_T_TMP_DIR}/${SAMPLE}_tumour.collate.tmp | \
samtools fastq -OT RG,BG -@ 2 - | \
bwa mem -p -t 2 -C ${REFERENCE} - | \
samtools sort -@ 2 -m 500m \
-T ${REMAP_HLA_T_TMP_DIR} \
-o ${REMAP_HLA_T_OUT} - \
&& samtools index ${REMAP_HLA_T_OUT} \
&& touch ${REMAP_HLA_T_DONE}
"
    echo "REMAP_HLA_T command is:"
    echo "${REMAP_HLA_T_CMD}" | tee "${LOGDIR}/${REMAP_HLA_T_JOB}.cmd"
    REMAP_HLA_T_BSUB=(bsub
			-M 10G
      -n 8
			-o "${LOGDIR}/${REMAP_HLA_T_JOB}.o"
			-e "${LOGDIR}/${REMAP_HLA_T_JOB}.e"
			-J "${REMAP_HLA_T_JOB}"
			"${REMAP_HLA_T_CMD}")
    echo "${REMAP_HLA_T_BSUB[@]}" > "${LOGDIR}/${REMAP_HLA_T_JOB}.bsub"
    "${REMAP_HLA_T_BSUB[@]}"
  fi
}

merge_tumour_function() {
  MERGE_HLA_T_OUT=${REMAP_HLA_T_OUT/.bam/.merged.bam}
  MERGE_HLA_T_DONE=${MERGE_HLA_T_OUT}.done
  MERGE_HLA_T_JOB="MERGE_HLA_T_${RAND}"
  MERGE_HLA_T_FLAG=true
  if [ -e ${MERGE_HLA_T_DONE} ]; then
		echo "MERGE_HLA_T output exists at ${MERGE_HLA_T_OUT}"
		MERGE_HLA_T_FLAG=false
	else
    MERGE_HLA_T_CMD="samtools merge -@ 8 -h ${TUMOUR_BAM} \
-L ${HLA_LOCI} ${MERGE_HLA_T_OUT} ${TUMOUR_BAM} ${REMAP_HLA_T_OUT} \
&& samtools index ${MERGE_HLA_T_OUT} \
&& touch ${MERGE_HLA_T_DONE}
"
    echo "MERGE_HLA_T command is:"
    echo "${MERGE_HLA_T_CMD}" | tee "${LOGDIR}/${MERGE_HLA_T_JOB}.cmd"
    MERGE_HLA_T_BSUB=(bsub
			-M 4G
      -n 8
			-o "${LOGDIR}/${MERGE_HLA_T_JOB}.o"
			-e "${LOGDIR}/${MERGE_HLA_T_JOB}.e"
			-J "${MERGE_HLA_T_JOB}"
    )
    if [ "${REMAP_HLA_T_FLAG}" = "true" ]; then
			MERGE_HLA_T_BSUB+=("-w done(${REMAP_HLA_T_JOB})")
		fi
		MERGE_HLA_T_BSUB+=("${MERGE_HLA_T_CMD}")
    echo "${MERGE_HLA_T_BSUB[@]}" > "${LOGDIR}/${MERGE_HLA_T_JOB}.bsub"
    "${MERGE_HLA_T_BSUB[@]}"
  fi
}
...
echo ""
echo "###Preparing REMAP_HLA_T"
remap_tumour_function
echo "###"
echo ""
echo "###Preparing MERGE_HLA_T"
merge_tumour_function
echo "###"
echo ""
